...
--관리자계정 추가
insert into member
values('admin2','1234','관리자','A','M', to_date('19971020','yyyymmdd'), 'admin@naver.com', '01044441234', '게임,독서',default, default);
...
create table board(
                      ...
                      constraints fk_board_member_id foreign key(member_id)
...
--첨부파일은 서버에 저장하고, 저장된 파일에 대한 정보만 db테이블에 저장
create table attachment(
                           ...
                           constraints fk_attachment_board_id foreign key(board_id)
...
insert into web.board (id,title,member_id,content,reg_date,read_count) values (seq_board_id.nextval,'비 좀 그만 내려라','hyejin','바지 다젖었어요 차라리 눈이 나은듯',default,0);
insert into web.board (id,title,member_id,content,reg_date,read_count) values (seq_board_id.nextval,'게시글수가 60개라 한 개 더 추가해본다.','hyejin','그것이 테스트니까.',default,0);
commit;
...
-- servertest 계정으로 TB_USER 테이블 생성 및 SEQ_UNO 시퀀스 생성
...
CREATE TABLE NOTICE(
                       ...
                       FOREIGN KEY (NOTICE_WRITER) REFERENCES MEMBER (USERID)
...
values(seq_board_comment_id.nextval, 367, 'abcde', '좋은글 잘 봤습니다.', default, null, default);
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'admin', '활력을 얻고 갑니다', default, null, default);
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'sejong', '이달의 글로 선정되었습니다.', default, null, default);
commit;

-- 대댓글
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'sejong4', '읽어주셔서 감사합니다~ 구독과 좋아요 부탁드려요~', 2, 21, default);
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'sejong4', '활력을 어디서 얻으셨어요?', 2, 22, default);
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'sejong4', '감사합니다~?', 2, 23, default);
insert into board_comment
values(seq_board_comment_id.nextval, 367, 'admin', '첨부파일이 훌륭합니다.', 2, 22, default);
...
create table photo (
                       ...
                       constraints fk_photo_member_id foreign key(member_id) references member(id) on delete set null
);
-- drop table photo;
